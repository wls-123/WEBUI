new Vue({
  ...
  methods: {
    ...
    submitArticle(){
      $.ajax({
        url: 'http://120.78.72.35:8099/manager/article/saveOrUpdateArticle',
        method: 'post',
    ...
    deleteArticle(index, rows) {
      // console.log(rows.id)
      $.ajax({
        url: "http://120.78.72.35:8099/manager/article/deleteArticleById",
        method: "get",
        data: { id: rows.id },
        success: (res) => {
          alert("删除成功");
    ...
    loadCategory() {
      $.ajax({
        url: 'http://120.78.72.35:8099/manager/category/findAllCategory',
        method: 'get',
        data: {},
        success: (res) => {
    ...
    checkArticle(item) {
      ...
      // 调用接口
      $.ajax({
        url: 'http://120.78.72.35:8099/manager/article/checkArticle',
        method: 'get',
        data: obj,
        success: (res) => {
    ...
    loadArticle() {
      $.ajax({
        url: 'http://120.78.72.35:8099/manager/article/findArticle',
        method: 'get',
        data: this.params,
        success: (res) => {
    ...
    loginHandler() {
      $.ajax({
        url: 'http://120.78.72.35:8099/manager/user/login',
        method: 'post',
    ...
    findMsg(token) {
      $.ajax({
        url: 'http://120.78.72.35:8099/manager/user/info',
        method: 'get',
        data: { token },
        success: (res) => {
